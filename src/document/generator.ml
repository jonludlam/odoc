...
module Make (Syntax : SYNTAX) = struct
  ...
  module Source_page : sig
    ...
    let info_of_info : Lang.Source_info.annotation -> Source_page.info option = function
      | Value id -> (
      ...
      | Definition id -> (
        match id.iv with
        | `SourceLocation (_, def) ->
          Some (Anchor (DefName.to_string def))
    ...
    let source id syntax_info infos source_code =
      let url = path id in
      let mapper (info, loc) = match info_of_info info with | Some x -> Some (x, loc) | None -> None in
