...
let rec block ~config ~resolve (l : Block.t) : flow Html.elt list =
  ...
  let one (t : Block.one) =
    ...
    match t.desc with
    | Inline i ->
        if a = [] then as_flow @@ inline ~config ~resolve i
        else [ Html.span ~a (inline ~config ~resolve i) ]
    | Paragraph i -> [ Html.p ~a (inline ~config ~resolve i) ]
    | List (typ, l) ->
        let mk = match typ with Ordered -> Html.ol | Unordered -> Html.ul in
        [ mk ~a (List.map (fun x -> Html.li (block ~config ~resolve x)) l) ]
    | Description l ->
        [
          (let item i =
    ...
    | Source c -> [ Html.pre ~a (source (inline ~config ~resolve) c) ]
  in
