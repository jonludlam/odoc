...
let careful_type_in_sig sg name =
  ...
  | Some _ as x -> x
  | None -> find_map removed_type sg.Signature.removed
...
let any_in_sig sg name =
  filter_in_sig sg (function
    ...
    | Type (id, _, t) when N.type_ id = name -> Some (`T (Delayed.get t))
    ...
    | Class (id, _, c) when N.class_ id = name -> Some (`C c)
    | ClassType (id, _, ct) when N.class_type id = name -> Some (`CT ct)
    | Type (id, _, t) -> (
...
let any_in_type_in_sig sg name =
  filter_in_sig sg (function
    | Signature.Type (id, _, t) -> (
        ...
        | Some x -> Some (`In_type (typename_of_typeid id, t, x))
        | None -> None )
    | _ -> None)
