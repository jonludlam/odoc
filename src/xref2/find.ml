...
let type_in_sig sg name =
  find_in_sig sg (function
    ...
    | Class (id, _, c) when TypeName.equal_modulo_shadowing (N.typed_class id) name ->
        Some (`FClass (N.class' id, c))
    | ClassType (id, _, c) when TypeName.equal_modulo_shadowing (N.typed_class_type id) name ->
...
let class_in_sig sg name =
  filter_in_sig sg (function
    | Signature.Class (id, _, c) when TypeName.equal_modulo_shadowing (N.typed_class id) name ->
        Some (`FClass (N.class' id, c))
    | Signature.ClassType (id, _, c) when TypeName.equal_modulo_shadowing (N.typed_class_type id) name ->
        Some (`FClassType (N.class_type' id, c))
    | _ -> None)
