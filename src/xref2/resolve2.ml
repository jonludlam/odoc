...
and module_type_path :
 ...
 let cp = Component.Of_Lang.(module_type_path empty p) in
  ...
  | Unresolved p ->
      Cpath.module_type_path_of_cpath p
  | exception e ->
      Format.fprintf Format.err_formatter
...
and class_type_expr env =
  ...
  | Signature s ->
      Signature (class_signature env s)
...
and class_signature env c =
  ...
  let map_item = function
    | Method m ->
        Method (method_ env m)
    | InstanceVariable i ->
        InstanceVariable (instance_variable env i)
    | Constraint (t1, t2) ->
        Constraint (type_expression env t1, type_expression env t2)
    | Inherit c ->
        Inherit (class_type_expr env c)
    | Comment c ->
        Comment c
  in
  {self= Opt.map (type_expression env) c.self; items= List.map map_item c.items}
...
and class_ env c =
  ...
  let rec map_decl = function
    | ClassType expr ->
        ClassType (class_type_expr env expr)
    | Arrow (lbl, expr, decl) ->
 ...
 fun env s ->
  ...
  List.map
    (fun item ->
      match item with
      | Module (r, m) ->
          Module (r, module_ env m)
      | ModuleSubstitution m ->
          ModuleSubstitution (module_substitution env m)
      | Type (r, t) ->
          Type (r, type_decl env t)
      | TypeSubstitution t ->
          TypeSubstitution (type_decl env t)
      | ModuleType mt ->
          ModuleType (module_type env mt)
      | Value v ->
          Value (value_ env v)
      | Comment c ->
      ...
      | Exception e ->
          Exception (exception_ env e)
      | External e ->
          External (external_ env e)
      | Class (r, c) ->
          Class (r, class_ env c)
      | ClassType (r, c) ->
          ClassType (r, class_type env c)
      | Include i ->
...
and type_decl_constructor_argument env c =
  ...
  match c with
  | Tuple ts ->
      Tuple (List.map (type_expression env) ts)
  | Record fs ->
 ...
 fun env texpr ->
  ...
  try
    ...
    | Var _ | Any ->
        texpr
    | Alias (t, str) ->
        Alias (type_expression env t, str)
    | Arrow (lbl, t1, t2) ->
    ...
    | Poly (strs, t) ->
        Poly (strs, type_expression env t)
    | Package p ->
