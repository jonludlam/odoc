...
and unresolve_class_type_path : class_type -> class_type = function
  ...
  let rec original_path_cpath : module_ -> module_ option =
    function
    | `Resolved p ->
  ...
  and original_path_module_identifier : Odoc_model.Paths.Identifier.Path.Module.t -> Resolved.module_ option = fun id ->
    match id.iv with
    | `Module (sg, name) -> (
  ...
  and original_path_parent_identifier : Odoc_model.Paths.Identifier.Signature.t -> Resolved.parent option = fun id ->
    match id with
  ...
  and original_path_module_type_identifier : Odoc_model.Paths.Identifier.ModuleType.t -> Resolved.module_type option = fun id ->
    match id.iv with
    | `ModuleType (sg, name) -> (
      match original_path_parent_identifier sg with
