...
module rec Resolved : sig
  ...
  and module_unhashed =
    ...
    | `AliasRD of module_ * Cpath.module_ * module_ option
  ...
  module Resolved = struct
    ...
    module Module = struct
      ...
      let gpath : Path.Resolved.Module.t -> Resolved.module_ =
        let tbl = Hashtbl.create 255 in
        fun x ->
          if Hashtbl.mem tbl x.Hc.key then Hashtbl.find tbl x.key
          else
            let y = Hc.mk (`Gpath x) in
            Hashtbl.add tbl x.key y;
            y
      ...
      let aliasrd : Resolved.module_ * module_ * Resolved.module_ option -> Resolved.module_ =
        Hc.gen3 (fun (x, y, z) -> `AliasRD (x, y, z))
...
let rec is_resolved_module_substituted : Resolved.module_ -> bool =
 fun x ->
  ...
  | `Hidden a | `Apply (a, _) | `AliasRD (a, _, _) | `Canonical (a, _) ->
 ...
 fun ~weak_canonical_test ->
  let rec inner : Resolved.module_ -> bool =
   fun x ->
    ...
    | `AliasRD (p1, { v = `Resolved p2; _ }, _) ->
        inner p1 && inner p2
    | `AliasRD (p1, _p2, _) -> inner p1
  ...
  | `Module (parent, name) ->
      Module.module_
        ( Parent.module_ (resolved_module_of_resolved_signature_reference parent),
          name )
  | `Identifier x ->
      Module.gpath (Odoc_model.Paths.Path.Resolved.Module.Mk.identifier x)
...
let rec unresolve_resolved_module_path : Resolved.module_ -> module_ =
  ...
  fun x ->
    match x.v with
    | `Hidden { v = `Gpath { v = `Identifier x; _ }; _ } -> identifier (x, true)
    | `Gpath { v = `Identifier x; _ } -> identifier (x, false)
    | `Gpath _ -> resolved x
    ...
    | `AliasRD (_, { v = `Resolved m; _ }, _) -> unresolve_resolved_module_path m
    | `AliasRD (_, m, _) -> m
