...
let rec resolved_module_path_of_cpath : resolved_module -> Path.Resolved.Module.t =
    function
...
and resolved_module_type_path_of_cpath : resolved_module_type -> Path.Resolved.ModuleType.t =
    function
...
and resolved_type_path_of_cpath : resolved_type -> Path.Resolved.Type.t =
    function
    | `Identifier (#Odoc_model.Paths_types.Identifier.path_type as x) -> `Identifier x
...
and module_path_of_cpath : module_ -> Path.Module.t =
    function
...
let rec is_resolved_module_substituted : resolved_module -> bool =
    ...
    | `Apply (a, _)
    | `Alias (a, _) 
    | `Module (a, _) -> is_resolved_module_substituted a
...
and is_resolved_type_substituted : resolved_type -> bool =
    function
    | `Local _ -> false
    | `Substituted _ -> true
    | `Identifier _ -> false
    | `Type (a, _)
    | `Class (a, _)
    | `ClassType (a, _) -> is_resolved_module_substituted a

and is_resolved_class_type_substituted : resolved_class_type -> bool =
    function
    | `Local _ -> false
    | `Substituted _ -> true
    | `Identifier _ -> false
...
let rec is_module_substituted : module_ -> bool =
    ...
    | `Substituted _ -> true
    | `Dot(a,_)
    | `Apply(a, _) -> is_module_substituted a
    | `Forward _ -> false
    | `Root _ -> false
...
let rec is_module_hidden : module_ -> bool =
    ...
    | `Substituted p
    | `Dot(p,_)
    | `Apply(p,_) -> is_module_hidden p
    | `Forward _ -> false
    | `Root _ -> false

and is_resolved_module_hidden : resolved_module -> bool =
    function
    | `Local _ -> false
    | `Identifier _ -> false
    | `Hidden _ -> true
    ...
    | `Canonical (p, _)
    | `Apply (p, _)
    | `Module (p, _) -> is_resolved_module_hidden p
    | `Subst (p1, p2) -> is_resolved_module_type_hidden p1 || is_resolved_module_hidden p2
    | `SubstAlias (p1, p2)
    | `Alias (p1, p2) -> is_resolved_module_hidden p1 || is_resolved_module_hidden p2
...
and is_resolved_module_type_hidden : resolved_module_type -> bool =
    function
    | `Local _ -> false
    | `Identifier _ -> false
...
and is_resolved_type_hidden : resolved_type -> bool =
    function
    | `Local _ -> false
    | `Identifier _ -> false
    ...
    | `Type (p, _)
    | `Class (p, _)
    | `ClassType (p, _) -> is_resolved_module_hidden p

and is_resolved_class_type_hidden : resolved_class_type -> bool =
    function
    | `Local _ -> false
    | `Identifier _ -> false
