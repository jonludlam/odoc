...
let check_ambiguous_label ~loc env
    (attrs, ({ Odoc_model.Hc.v = `Label (_, label_name); _ } as id), _) =
...
and should_resolve : Paths.Path.t -> bool =
 fun p -> match p.v with `Resolved p -> should_reresolve p | _ -> true
  ...
  | `Reference (r, content) as orig -> (
      ...
      | Ok x ->
          let content =
            ...
            | [], `Identifier ({ v = #Id.Label.t_unhashed; _ } as i) -> (
...
and type_decl : Env.t -> Id.Signature.t -> TypeDecl.t -> TypeDecl.t =
 fun env parent t ->
  ...
  let hidden_path =
    ...
    | Some (Constr ({ v = `Resolved path; _ }, params))
        ...
        | Ok (cp', `FType (_, t)) ->
            ...
            if List.mem p visited then raise Loop
              ...
              | { manifest = Some expr; params; _ } -> (
                  ...
                  with
                  | Loop -> Constr (Odoc_model.Paths.Path.Type.Mk.resolved p, ts)
                  | e ->
                      Format.eprintf
              ...
              | _ -> Constr (Odoc_model.Paths.Path.Type.Mk.resolved p, ts)
            else Constr (Odoc_model.Paths.Path.Type.Mk.resolved p, ts)
