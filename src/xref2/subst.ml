...
and resolved_signature_reference :
 ...
 fun t r ->
  match r with
  | (`Local #Ident.module_ | #Cref.Resolved.module_no_id) as s ->
      (resolved_module_reference t s :> Cref.Resolved.signature)
  | `Local (`LModuleType _ as id) -> (
      match
        try Some (ModuleTypeMap.find id t.ref_module_type) with _ -> None
      with
...
and resolved_class_signature_reference :
 ...
 fun t r ->
  ...
  | `Local (#Ident.class_signature as c) -> (
      match
        try Some (ClassTypeMap.find c t.ref_class_type) with _ -> None
      with
...
and resolved_type_reference : t -> Cref.Resolved.type_ -> Cref.Resolved.type_ =
 fun t r ->
  match r with
  | (`Local #Ident.class_signature | `Class _ | `ClassType _) as c ->
      (resolved_class_signature_reference t c :> Cref.Resolved.type_)
...
and resolved_parent_reference :
 ...
 fun t r ->
  match r with
  | (`Local #Ident.signature | #Cref.Resolved.signature_no_id) as l ->
      (resolved_signature_reference t l :> Cref.Resolved.parent)
  | `Identifier _ -> r
  | (`Local #Ident.path_type | `Type (_, _)) as ty ->
      (resolved_type_reference t ty :> Cref.Resolved.parent)
  | #Cref.Resolved.class_signature_no_id as c ->
      (resolved_class_signature_reference t c :> Cref.Resolved.parent)

and resolved_label_parent_reference :
    t -> Cref.Resolved.label_parent -> Cref.Resolved.label_parent =
 fun t r ->
  match r with
  | (#Cref.Resolved.parent_no_id | `Local #Ident.parent) as l ->
      (resolved_parent_reference t l :> Cref.Resolved.label_parent)
  | `Identifier _ -> r
  | `Local (`LPage _) -> r

and resolved_reference : t -> Cref.Resolved.any -> Cref.Resolved.any =
 fun t r ->
  match r with
  | (`Local #Ident.parent | #Cref.Resolved.parent_no_id) as p ->
      (resolved_parent_reference t p :> Cref.Resolved.any)
  ...
  | `InstanceVariable (p, name) ->
      `InstanceVariable (resolved_class_signature_reference t p, name)
...
and reference : t -> Cref.any -> Cref.any =
 fun t r ->
  ...
  | (`Dot _ | `Module _ | `ModuleType _ | `Class _ | `ClassType _ | `Type _) as
    r ->
      (label_parent_reference t r :> Cref.any)
  | `Root _ -> r
  | `Constructor _ -> r (* FIXME *)
  | `Field _ -> r (* FIXME *)
...
and module_expansion s t =
  ...
  | Functor (arg, sg) ->
      Functor (List.map (functor_argument_opt s) arg, signature s sg)
...
and nestable_block_element :
 ...
 fun s n ->
  ...
  | `Code_block _ | `Verbatim _ -> n
  | `List (x, yss) -> `List (x, list (list nestable_block_element) s yss)
  | `Modules ms -> `Modules (list module_reference s ms)

and tag : t -> Component.CComment.tag -> Component.CComment.tag =
 fun s t ->
  match t with
  | `Author _ | `Inline | `Open | `Closed | `Since _ | `Version _ -> t
...
and block_element :
    t -> Component.CComment.block_element -> Component.CComment.block_element =
 fun s t ->
  match t with
  | #Component.CComment.nestable_block_element as x ->
      (nestable_block_element s x :> Component.CComment.block_element)
  | `Heading (lvl, id, l) ->
      let id' =
        match
          try Some (IdentMap.find (id :> Ident.any) s.id_any) with _ -> None
        with
...
and docs_or_stop s = function `Docs d -> `Docs (docs s d) | `Stop -> `Stop
