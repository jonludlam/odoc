      ...
      (fun (items, s) item ->
        ...
        | Module (id, r, m) ->
            ...
            let canonical =
              ...
              | Some p -> Some (Odoc_model.Paths.Path.Module.Mk.dot (p, name))
              | None -> None
            in
            let m' =
              Component.Delayed.(
        ...
        | ModuleType (id, mt) ->
            ( ModuleType
                ( id,
                  Component.Delayed.(Strengthen 
                    (ModuleType,
                      mt,
                        (Cpath.Mk.ModuleType.dot
                           (prefix, Ident.Name.module_type id)), None)))
              :: items,
              s )
        | Type (id, r, t) ->
            ( Type
                ( id,
                  r,
                  Component.Delayed.(Strengthen (Type, t, 
