...
module Fmt = struct
  type config = {
    ...
    identifier_name_only : bool;
        (** Print Identifier-wrapped paths using only the leaf name (as the
            HTML renderer does). When canonical resolution has lifted a
            path like [Mylib.A] into a single Identifier, this prints just
            [A]. Has no effect on Module-step paths where the structure
            inherently requires walking the chain. *)
  }
   ...
   fun c ppf sg ->
    ...
    let sig_item ppf = function
      | Module (id, _, m) ->
          ...
          let hidden_name =
            ModuleName.is_hidden (Ident.Name.typed_module id)
          in
          let lb, rb =
  ...
  and is_empty_sg (sg : Signature.t) =
    sg.items = [] && sg.removed = []
  ...
  and type_equation ?sep c ppf t =
    Format.fprintf ppf "%a%a" (type_equation_params c) t
      (type_equation_manifest ?sep c) t
  ...
  and type_expr c ppf e =
    ...
    in
    let pp_atom ppf t =
      if needs_outer_parens t then
        Format.fprintf ppf "(%a)" (type_expr c) t
    ...
    | Arrow (l, t1, t2) ->
        ...
        (match t1 with
         | Arrow _ ->
             Format.fprintf ppf "%a(%a) -> %a" type_expr_label l (type_expr c)
               t1 (type_expr c) t2
         | _ ->
    ...
    | Constr (p, args) -> (
        ...
        | _ ->
            Format.fprintf ppf "(%a) %a" (type_expr_list c) args
              (type_path c) p)
   ...
   fun c ppf (p : path) ->
    ...
    | `Identifier (id, _) when c.identifier_name_only ->
        Format.fprintf ppf "%s"
          (Odoc_model.Paths.Identifier.name (id :> id))
    | `Identifier (id, b) ->
        wrap2 c "identifier" model_identifier bool ppf (id :> id) b
