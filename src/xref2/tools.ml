...
module LookupParentMemo = MakeMemo (struct
  type t = bool * bool * Cpath.Resolved.parent
...
let rec handle_apply ~mark_substituted env func_path arg_path dm =
  let rec find_functor mty =
    ...
    | Component.ModuleType.Path { p_path; _ } -> (
        ...
        | Ok (_, dt) -> (
            ...
            | { Component.ModuleType.expr = Some mty'; _ } -> find_functor mty'
            | _ -> Error `OpaqueModule)
        | _ -> Error `OpaqueModule)
    | _ -> Error `ApplyNotFunctor
  in
  module_type_expr_of_module env dm >>= fun mty' ->
  find_functor mty' >>= fun (arg_id, result) ->
  let new_module = { (Component.dget dm) with Component.Module.type_ = ModuleType result } in
  let substitution =
...
and process_module_path env ~add_canonical md rp =
  let rp = if Dhelpers.Module.hidden md then `Hidden rp else rp in
  let rp' =
...
and handle_module_lookup env ~add_canonical id rparent sg sub =
  ...
  | Some (`FModule (name, m)) ->
      ...
      let md' = Component.Delayed.(Subst (Module, m, sub)) in
      Ok (process_module_path env ~add_canonical md' rp', md')
  | Some (`FModule_removed p) ->
      lookup_module ~mark_substituted:false ~shortcut_alias:false env p >>= fun m -> Ok (p, m)
...
and handle_module_type_lookup env ~add_canonical id p sg sub =
  ...
  let mt = Component.Delayed.(Subst (ModuleType, mt, sub)) in
 ...
 fun ~mark_substituted env path ->
  ...
  | `Module (parent, name) ->
      let find_in_sg sg sub =
        ...
        | None -> Error `Find_failure
        | Some (`FModule (_, m)) ->
            Ok Component.Delayed.(Subst (Module, m, sub))
        | Some (`FModule_removed p) -> lookup_module ~mark_substituted ~shortcut_alias:false env p
      in
...
and lookup_module :
    ...
    ( Component.Module.t Component.Delayed.t,
      simple_module_lookup_error )
    Result.result =
 fun ~mark_substituted:m ~shortcut_alias:s env' path' ->
  let lookup env (mark_substituted, shortcut_alias, (path : ExpansionOfModuleMemo.M.key)) =
    match path with
    | `Local lpath -> Error (`Local (env, lpath))
    | `Gpath p -> lookup_module_gpath ~mark_substituted env p
    | `Substituted x -> lookup_module ~mark_substituted ~shortcut_alias env x
    | `Apply (functor_path, argument_path) ->
        lookup_module ~mark_substituted ~shortcut_alias env functor_path
        >>= fun functor_module ->
    ...
    | `Module (parent, name) ->
        let find_in_sg sg sub =
          ...
          | Some (`FModule (_, m)) ->
              Ok (Component.Delayed.Subst (Module, m, sub))
          | Some (`FModule_removed p) -> lookup_module ~mark_substituted ~shortcut_alias env p
        in
        lookup_parent ~mark_substituted ~shortcut_alias env parent
        |> map_error (fun e -> (e :> simple_module_lookup_error))
        >>= fun (sg, sub) -> find_in_sg sg sub
    | `Alias (p, _, _) when shortcut_alias -> lookup_module ~mark_substituted ~shortcut_alias env p
    | `Alias (_, cs, _) -> (
        match resolve_module ~mark_substituted ~add_canonical:false env cs with
        | Ok (_, r) -> Ok r
        | Error e -> Error e)
    | `Subst (_, p) -> lookup_module ~mark_substituted ~shortcut_alias env p
    | `Hidden p -> lookup_module ~mark_substituted ~shortcut_alias env p
    | `Canonical (p, _) -> lookup_module ~mark_substituted ~shortcut_alias env p
    | `OpaqueModule m -> lookup_module ~mark_substituted ~shortcut_alias env m
  in
  LookupModuleMemo.memoize lookup env' (m, s, path')
  ...
  | `ModuleType (parent, name) ->
      let find_in_sg sg sub =
        ...
        | Some (`FModuleType (_, mt)) ->
            Ok Component.Delayed.(Subst (ModuleType, mt, sub))
      in
      lookup_parent_gpath ~mark_substituted env parent
    ...
    | `ModuleType (parent, name) ->
        let find_in_sg sg sub =
          ...
          | Some (`FModuleType (_, mt)) ->
              Ok Component.Delayed.(Subst (ModuleType, mt, sub))
        in
        lookup_parent ~mark_substituted:true ~shortcut_alias:false env parent
        |> map_error (fun e -> (e :> simple_module_type_lookup_error))
        >>= fun (sg, sub) -> find_in_sg sg sub
...
and lookup_parent :
    ...
    ( Component.Signature.t * Component.Substitution.t,
      [ `Parent of parent_lookup_error ] )
    Result.result =
 fun ~mark_substituted:m ~shortcut_alias:s env' parent' ->
  let lookup env (mark_substituted, shortcut_alias, parent) =
    match parent with
    | `Module p ->
        lookup_module ~mark_substituted ~shortcut_alias env p
    ...
    | `ModuleType p ->
        ...
        >>= fun dmt ->
        expansion_of_module_type env dmt
        |> map_error (fun e -> `Parent (`Parent_sig e))
        >>= assert_not_functor
        >>= fun sg -> Ok (sg, prefix_substitution parent sg)
    | `FragmentRoot ->
  ...
  in
  LookupParentMemo.memoize lookup env' (m, s, parent')
...
and lookup_type_gpath :
 ...
 fun env p ->
  let do_type p name =
    ...
    | Some (`FType (name, t)) ->
        Ok (`FType (name, Component.Delayed.(Subst (Type, t, sub))))
...
and lookup_type :
 ...
 fun env p ->
  let do_type p name =
    lookup_parent ~mark_substituted:true ~shortcut_alias:false env p
    |> map_error (fun e -> (e :> simple_type_lookup_error))
    >>= fun (sg, sub) ->
...
and resolve_module :
 ...
 fun ~mark_substituted ~add_canonical env' path ->
  ...
  let resolve env (mark_substituted, add_canonical, p) =
    ...
    | `Module (parent, id) ->
        lookup_parent ~mark_substituted ~shortcut_alias:false env parent
        |> map_error (fun e -> (e :> simple_module_lookup_error))
        >>= fun (parent_sig, sub) ->
    ...
    | `Resolved r -> lookup_module ~mark_substituted ~shortcut_alias:false env r >>= fun m -> Ok (r, m)
    | `Substituted s ->
...
and handle_canonical_module_real env p2 =
  ...
  | Some (rp, dm) ->
      ...
      let expanded =
        match Dhelpers.Module.type_ dm with
        ...
        | Alias (`Resolved p, None) ->
            ...
            let rec check dm =
              match Dhelpers.Module.canonical dm with
              | Some p ->
                  p = p2
              ...
              | None -> (
                  ...
                  | Component.Module.Alias (`Resolved p, _) -> (
                      match lookup_module ~mark_substituted:false ~shortcut_alias:false env p with
                      | Error _ -> false
                      | Ok dm ->
                          check dm)
                  | _ -> false)
            in
            let self_canonical () = check dm in
            let hidden =
...
and module_type_expr_of_module_decl :
 ...
 fun env decl ->
  ...
  | Component.Module.Alias (`Resolved r, _) ->
      lookup_module ~mark_substituted:false ~shortcut_alias:false env r
      |> map_error (fun e -> `Parent (`Parent_module e))
      >>= fun m ->
      module_type_expr_of_module_decl env (Dhelpers.Module.type_ m)
...
and module_type_expr_of_module :
    ...
    ( Component.ModuleType.expr,
      simple_module_type_expr_of_module_error )
    Result.result =
 fun env m -> module_type_expr_of_module_decl env (Dhelpers.Module.type_ m)
...
and expansion_of_module_path :
 ...
 fun env ~strengthen path ->
  let expand (p', m) =
    ...
    | Signature sg ->
        let sg' =
          match Dhelpers.Module.doc m with
          | [] -> sg
  ...
  let handle_unresolved path =
    ...
    | Error e -> Error (`UnresolvedPath (`Module (path, e)))
  in
  let handle_resolved rp =
    match lookup_module ~mark_substituted:true ~shortcut_alias:true env rp with
    | Ok m -> expand (rp, m)
    | Error _ when Cpath.is_module_forward path -> Error `UnresolvedForwardPath
    | Error e -> Error (`UnresolvedPath (`Module (path, e)))
  in
  match path with
  | `Resolved rp -> handle_resolved rp
  | _ -> handle_unresolved path
...
and signature_of_u_module_type_expr :
 ...
 fun ~mark_substituted env m ->
  ...
  | Component.ModuleType.U.Path p -> (
      ...
      | Ok (_, dmt) ->
          expansion_of_module_type env dmt >>= assert_not_functor
...
and expansion_of_module_type :
    ...
    (expansion, expansion_of_module_error) Result.result =
 fun env m ->
  match Dhelpers.ModuleType.expr m with
...
and expansion_of_module :
    ...
    (expansion, expansion_of_module_error) Result.result =
 fun env m -> 
  match Dhelpers.Module.type_ m with
...
and find_module_with_replacement :
 ...
 fun env sg name ->
  ...
  | Some (`FModule_removed path) ->
      lookup_module ~mark_substituted:false ~shortcut_alias:false env path
  | None -> Error `Find_failure
...
let resolve_module_type_path env p =
  ...
  >>= fun (p, dmt) ->
  match expansion_of_module_type env dmt with
  | Ok _ -> Ok p
