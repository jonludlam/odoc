...
let rec dget_impl : type a. a Component.Delayed.t -> a = function
  ...
  | Strengthen (Module, m, p, canonical) -> Strengthen.module_ ?canonical p (dget_impl m)
...
let _ =
  Component.dget_impl := Some { Component.dget = dget_impl }
...
let core_types =
  ...
  List.map
    (fun decl ->
      (Identifier.name decl.id, Component.Delayed.(OfLang (Type, decl, (Component.Of_Lang.empty ())))))
    ...
    List.map
      (function
        ...
        | Value (id, v) ->
            Value
              ( id,
                Component.Delayed.(Subst (Value, v, sub)))
        | Class (id, r, c) ->
...
module LookupModuleMemo = MakeMemo (struct
  ...
  let equal (x1, x2) (y1, y2) =
    x1 = y1 && x2.Odoc_model.Hc.key = y2.Odoc_model.Hc.key

  let hash (x, y) = Hashtbl.hash (x, y.Odoc_model.Hc.key)
end)
...
module LookupAndResolveMemo = MakeMemo (struct
  ...
  let equal (x1, x2, x3) (y1, y2, y3) =
    x1 = y1 && x2 = y2 && x3.Odoc_model.Hc.key = y3.Odoc_model.Hc.key

  let hash (x, x1, y) = Hashtbl.hash (x, x1, y.Odoc_model.Hc.key)
end)
...
module SignatureOfModuleMemo = MakeMemo (struct
  ...
  let equal x y = x.Odoc_model.Hc.key = y.Odoc_model.Hc.key

  let hash x = Hashtbl.hash x.Odoc_model.Hc.key
end)
...
let simplify_module : Env.t -> Cpath.Resolved.module_ -> Cpath.Resolved.module_
 ...
 fun env m ->
  ...
  match m.v with
  | `Module ({ v = `Module { v = `Gpath { v = `Identifier p; _ }; _ }; _ }, name)
    -> (
      ...
      | Some _ ->
          Cpath.Mk.Resolved.Module.gpath
            (Odoc_model.Paths.Path.Resolved.Module.Mk.identifier ident)
      | None -> m)
  | _ -> m
...
let simplify_module_type :
 ...
 fun env m ->
  ...
  match m.v with
  | `ModuleType
      ({ v = `Module { v = `Gpath { v = `Identifier p; _ }; _ }; _ }, name) -> (
      let ident =
      ...
      | Some _ ->
          Cpath.Mk.Resolved.ModuleType.gpath
            (Odoc_model.Paths.Path.Resolved.ModuleType.Mk.identifier ident)
      | None -> m)
  | _ -> m
...
let simplify_type : Env.t -> Cpath.Resolved.type_ -> Cpath.Resolved.type_ =
 fun env m ->
  let open Odoc_model.Paths.Identifier in
  match m.v with
  | `Type ({ v = `Module { v = `Gpath { v = `Identifier p; _ }; _ }; _ }, name)
    -> (
      let ident = (Mk.type_ ((p :> Signature.t), name) : Path.Type.t) in
      match Env.(lookup_by_id s_type (ident :> Path.Type.t) env) with
      | Some _ ->
          Cpath.Mk.Resolved.Type.gpath
            (Odoc_model.Paths.Path.Resolved.Type.Mk.identifier ident)
      | None -> m)
  | _ -> m
  ...
  let rec find_functor mty =
    ...
    | Component.ModuleType.Path { p_path; _ } -> (
        ...
        | Ok (_, dt) -> begin
            ...
            | { Component.ModuleType.expr = Some mty'; _ } ->
              find_functor mty'
...
and lookup_module_type_gpath :
    ...
    (Component.ModuleType.t Component.Delayed.t, simple_module_type_lookup_error) Result.result =
 fun ~mark_substituted env path ->
  match path.v with
  | `Identifier i ->
  ...
  | `ModuleType (parent, name) ->
      let find_in_sg sg sub =
        ...
        | None -> Error `Find_failure
        | Some (`FModuleType (_, mt)) -> Ok (Component.Delayed.(Subst (ModuleType, mt, sub)))
      in
      lookup_parent_gpath ~mark_substituted env parent
...
and lookup_parent :
 ...
 fun ~mark_substituted:m env' parent' ->
  let lookup env (mark_substituted, parent) =
    match parent.Odoc_model.Hc.v with
    | `Module p ->
...
and lookup_type_gpath :
 ...
 fun env p ->
  let do_type p name =
    ...
    | Some (`FType (name, t)) -> Ok (`FType (name, Component.Delayed.(Subst (Type, t, sub))))
 ...
 fun env p ->
  let do_type p name =
    ...
    let t =
      ...
      | `FType (name, t) -> `FType (name, Component.Delayed.(Subst (Type, t, sub)))
  ...
  let resolve : Env.t -> bool * bool * Cpath.module_ -> _ =
   fun env (mark_substituted, add_canonical, p) ->
    ...
    | `Identifier (i, hidden) ->
        ...
        let rp =
          if hidden then
            Cpath.Mk.Resolved.Module.(
              hidden
                (gpath (Odoc_model.Paths.Path.Resolved.Module.Mk.identifier i)))
          else
            Cpath.Mk.Resolved.Module.gpath
              (Odoc_model.Paths.Path.Resolved.Module.Mk.identifier i)
        in
    ...
    | `Root r -> (
        ...
        | Some (Env.Resolved (_, i, m)) ->
            let rp =
              Cpath.Mk.Resolved.Module.gpath
                Odoc_model.Paths.(
                  Path.Resolved.Module.Mk.identifier
                    (i :> Identifier.Path.Module.t))
            in
            let p = process_module_path env ~add_canonical (Component.Delayed.Val m) rp in
...
and resolve_module_type :
 ...
 fun ~mark_substituted ~add_canonical env p ->
  ...
  | `Identifier (i, _) ->
      ...
      let p =
        Cpath.Mk.Resolved.ModuleType.gpath
          (Odoc_model.Paths.Path.Resolved.ModuleType.Mk.identifier i)
      in
      let p' = process_module_type env ~add_canonical mt p in
      Ok (p', mt)
 ...
 fun env ~add_canonical p ->
  let result =
    ...
    | `ClassType (parent, id) ->
        ...
        Ok (p', t)
    | `Identifier (i, _) ->
        let i' = Odoc_model.Paths.Path.Resolved.Type.Mk.identifier i in
  ...
  if add_canonical then
    ...
    | `FType (_, dt) -> begin
      match Component.dget dt with
      | { canonical = Some c; _ } ->
        Ok (Cpath.Mk.Resolved.Type.canonicaltype (p, c), t)
      | _ ->  result end
    | _ -> result
  else result
...
and resolve_class_type : Env.t -> Cpath.class_type -> resolve_class_type_result
 ...
 fun env p ->
  ...
  | `Dot (parent, id) ->
      ...
      Ok (p', t)
  | `Identifier (i, _) ->
      let i' = Odoc_model.Paths.Path.Resolved.ClassType.Mk.identifier i in
...
and reresolve_module_gpath :
    ...
    Odoc_model.Paths.Path.Resolved.Module.t =
 fun env path ->
  let open Odoc_model.Paths.Path.Resolved.Module.Mk in
  match path.v with
  | `Identifier _ -> path
  | `Apply (functor_path, argument_path) ->
      apply
  ...
  | `Hidden p ->
      ...
      hidden p'
  | `Canonical (p, ({ v = `Resolved _; _ } as p2)) ->
...
and reresolve_module_type_gpath :
    ...
    Odoc_model.Paths.Path.Resolved.ModuleType.t =
 fun env path ->
  let open Odoc_model.Paths.Path.Resolved.ModuleType.Mk in
  match path.v with
  | `Identifier _ -> path
  | `ModuleType (parent, name) ->
      module_type (reresolve_module_gpath env parent, name)
  | `CanonicalModuleType (p1, ({ v = `Resolved _; _ } as p2)) ->
...
and signature_of_u_module_type_expr :
 ...
 fun ~mark_substituted env m ->
  ...
  | Component.ModuleType.U.Path p -> (
      ...
      | Ok (_, dmt) -> let mt = Component.dget dmt in signature_of_module_type env mt
...
let resolve_module_path env p =
  ...
  >>= fun (p, m) ->
  match p.v with
  | `Gpath { v = `Identifier { iv = `Root _; _ }; _ }
  | `Hidden { v = `Gpath { v = `Identifier { iv = `Root _; _ }; _ }; _ } ->
      Ok p
  | _ -> (
      let m = Component.dget m in
