      ...
      | `ModuleType (parent, name) -> begin
        let sg =
          ...
          | `FragmentRoot ->
            ...
            sg
        in
        Find.module_type_in_sig sg name
        end
    in lookup env
      ...
      let rec find = function
        | Component.Signature.Module (id, _, m') :: _
          when Ident.Name.module_ id = name ->
            (id, `Module (`Module ppath, name), Subst.delayed_get_module m')
        | _ :: xs -> find xs
        | [] -> failwith "Can't find it"
      in
      find sg.items
          ...
          when Ident.Name.module_ id = ModuleName.to_string name -> (
            ...
            | Left m ->
                ( Component.Signature.Module
                    (id, r, Component.(Substitution.NoSubst (Delayed.put (fun () -> m))))
                  :: items,
                  true,
                  removed )
            | Right p ->
          ...
          (fun item (items, handled, removed) ->
            ...
            | Component.Signature.Type (id, r, t)
              when Ident.Name.type_ id = name -> (
                ...
                | Left x ->
                    (Component.Signature.Type (id, r, (Subst.make_delayed (fun () -> x))) :: items, true, removed)
                | Right y ->
          ...
          (fun item (items, handled) ->
            ...
            | Component.Signature.Module (id, r, m)
              when Ident.Name.module_ id = ModuleName.to_string name ->
                ...
                let item =
                  Component.Signature.Module
                    (id, r, Component.(Substitution.NoSubst (Delayed.put (fun () -> mapfn m))))
                in
                (item :: items, true)
            | Component.Signature.Include ({ expansion_; _ } as i) ->
...
and resolve_mt_type_fragment :
  ...
  | `Dot (parent, name) ->
    ...
    let result = fixup_type_cfrag (`Type (pfrag, name)) in
