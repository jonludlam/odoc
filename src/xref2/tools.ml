...
let rec dget_impl : type a. a Component.Delayed.t -> a = function
  ...
  | General ({ v=None; get = Some f} as g) -> let res = f () in g.v <- Some res; res
 ...
 fun ~mark_substituted:m env' parent' ->
  let lookup env (mark_substituted, parent) =
    ...
    | `Module p ->
        ...
        >>= assert_not_functor >>= fun sg -> Ok (sg, prefix_substitution parent sg)
    | `ModuleType p ->
...
and lookup_parent_gpath :
 ...
 fun ~mark_substituted env parent ->
  ...
  >>= fun m ->
  let m = Component.dget m in
  expansion_of_module env m |> map_error (fun e -> `Parent (`Parent_sig e))
  >>= assert_not_functor 
  >>= fun sg ->
  Ok
    ( sg,
...
and expansion_of_module_path :
 ...
 fun env ~strengthen path ->
  ...
  | Ok (p', m) -> begin
      ...
      | Signature sg ->
        let sg' = match m.doc with [] -> sg | docs -> { sg with items = Comment (`Docs docs) :: sg.items } in
        if strengthen then
          Ok (Signature (Strengthen.signature (Cpath.Mk.Module.resolved p') sg'))
...
and assert_not_functor : type err. expansion -> (Component.Signature.t, err) Result.t = function Signature sg -> Ok sg | _ -> assert false
...
and expansion_of_module_type_expr :
  ...
  *)
  | Component.ModuleType.With { w_substitutions; w_expr; _ } ->
      ...
      Ok (Signature sg)
  | Component.ModuleType.Functor (arg, expr) ->
      Ok (Functor (arg, expr))
...
and resolve_signature_fragment :
 ...
 fun env (p, sg) frag ->
  ...
  | `Dot (parent, name) ->
      ...
      of_result ResultMonad.(expansion_of_module env m' >>= assert_not_functor) >>= fun parent_sg ->
      let sg =
