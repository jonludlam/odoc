...
and handle_module_type_lookup env id p sg sub =
  let open OptionMonad in
  Find.module_type_in_sig sg id >>= fun (`MT mt) ->
  ...
  let lookup env =
    ...
    | `ModuleType (parent, name) ->
        let find_in_sg sg sub =
          ...
          | None -> Error `Find_failure
          | Some (`MT mt) -> Ok (Subst.module_type sub mt)
        in
...
and lookup_type :
    ...
    ( Find.careful_type, [ simple_type_lookup_error | parent_lookup_error ] )
    Result.result =
 fun env p ->
  let do_type p name =
        ...
        | Some (Env.Resolved (_, p, m)) ->
            ...
            return
              ( process_module_path env ~add_canonical m
                  (`Identifier
                    (p :> Odoc_model.Paths_types.Identifier.path_module)),
                m )
        | Some Env.Forward ->
...
and module_type_expr_of_module_decl :
 ...
 fun env decl ->
  ...
  | Component.Module.Alias (`Resolved r) ->
      ...
      >>= fun m -> module_type_expr_of_module_decl env m.type_
