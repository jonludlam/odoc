  ...
  let of_component env m base_path' base_ref' : t =
    ...
    let p, r =
      ...
      | Some (`Aliased cp) ->
          ...
          ( Module.aliasrd (cp, Cpath.Mk.Module.resolved base_path),
            `Alias (p, base_ref) )
      | Some (`SubstMT cp) ->
...
module DT = struct
  ...
  let of_component _env t ~parent_ref name = Ok (`Type (parent_ref, name), Component.dget t)

  let of_element _env (`Type (id, dt)) : t = let t = Component.dget dt in (`Identifier id, t)
