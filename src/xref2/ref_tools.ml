...
let ambiguous_ref_warning name results =
  ...
  Odoc_model.Error.implicit_warning
...
module M = struct
  ...
  let in_signature env ((parent, parent_cp, sg) : signature_lookup_result) name
    ...
    Find.module_in_sig sg (ModuleName.to_string name) >>= fun (`M m) ->
    Some
  ...
  let in_env env name : t option =
    match env_lookup_by_name Env.s_module (UnitName.to_string name) env with
...
module MT = struct
  ...
  let in_signature env ((parent', parent_cp, sg) : signature_lookup_result) name
      : t option =
    let sg = Tools.prefix_signature (parent_cp, sg) in
    Find.module_type_in_sig sg (ModuleTypeName.to_string name) >>= fun (`MT mt) ->
    Some
      (of_component env mt
  ...
  let in_env env name : t option =
    env_lookup_by_name Env.s_module_type (UnitName.to_string name) env
    >>= of_element env
end
...
module CL = struct
  ...
  let in_env env name =
    env_lookup_by_name Env.s_class (UnitName.to_string name) env >>= fun e ->
...
module CT = struct
  ...
  let in_env env name =
    env_lookup_by_name Env.s_class_type (UnitName.to_string name) env
...
module DT = struct
  ...
  let in_env env name : t option =
    env_lookup_by_name Env.s_datatype (UnitName.to_string name) env >>= function
  ...
  let in_signature _env ((parent', parent_cp, sg) : signature_lookup_result)
      name : t option =
    let sg = Tools.prefix_signature (parent_cp, sg) in
    Find.datatype_in_sig sg (TypeName.to_string name) >>= fun (`T t) ->
...
module V = struct
  ...
  let in_env env name : t option =
    env_lookup_by_name Env.s_value (UnitName.to_string name) env >>= function
...
module L = struct
  ...
  let in_env env name : t option =
    env_lookup_by_name Env.s_label (UnitName.to_string name) env
...
module EC = struct
  ...
  let in_env env name : t option =
    env_lookup_by_name Env.s_extension (UnitName.to_string name) env
...
module EX = struct
  ...
  let in_env env name : t option =
    env_lookup_by_name Env.s_exception (UnitName.to_string name) env
...
module CS = struct
  ...
  let in_env env name : t option =
    env_lookup_by_name Env.s_constructor (UnitName.to_string name) env
  ...
  let in_datatype _env ((parent', t) : datatype_lookup_result) name : t option =
    ...
    | `Cs _ -> Some (`Constructor (parent', name))
    | `Fd _ -> None
...
module F = struct
  ...
  let in_env env name : t option =
    env_lookup_by_name Env.s_field (UnitName.to_string name) env
  ...
  let in_parent _env (parent : label_parent_lookup_result) name : t option =
    ...
    | `T (parent', t) -> (
        ...
        | `Cs _ -> None
        | `Fd _ -> Some (`Field ((parent' :> Resolved.Parent.t), name)) )
...
module LP = struct
  ...
  let in_env env name : t option =
    env_lookup_by_name Env.s_label_parent (UnitName.to_string name) env
    >>= function
    | `Module _ as e ->
  ...
  fun env' r ->
    ...
    let resolve env =
      ...
      | `ModuleType (parent, name) ->
          ...
          >>= module_type_lookup_to_signature_lookup env
      | `Root (name, `TUnknown) -> (
          env_lookup_by_name Env.s_signature (UnitName.to_string name) env
          >>= function
          | `Module (_, _) as e ->
              M.of_element env e >>= module_lookup_to_signature_lookup env
          | `ModuleType (_, _) as e ->
...
let resolve_reference_dot_sg env ~parent_path ~parent_ref ~parent_sg name =
  ...
  | `T t ->
      DT.of_component env t ~parent_ref name
      >>= resolved2
  | `C c -> CL.of_component env c ~parent_ref name >>= resolved2
  | `CT ct -> CT.of_component env ct ~parent_ref name >>= resolved2
  | `V _ -> V.of_component env ~parent_ref name >>= resolved1
  | `E _ -> V.external_of_component env ~parent_ref name >>= resolved1
  | `L _ -> L.of_component env ~parent_ref name >>= resolved1
  | `Exn _ -> EX.of_component env ~parent_ref name >>= resolved1
  | `Ext _ -> EC.of_component env ~parent_ref name >>= resolved1
  | `In_type (typ_name, _, r) -> (
      let parent = `Type (parent_ref, typ_name) in
      match r with
      | `Cs _ -> CS.of_component env parent name >>= resolved1
      | `Fd _ -> F.of_component env parent name >>= resolved1 )
  | `Msub _ | `Tsub _ -> None
...
let resolve_reference_dot_type env ~parent_ref t name =
  ...
  | `Cs _ -> CS.of_component env parent_ref name >>= resolved1
  | `Fd _ -> F.of_component env parent_ref name >>= resolved1
...
let resolve_reference : Env.t -> t -> Resolved.t option =
  ...
  fun env r ->
    ...
    | `Root (name, `TUnknown) -> (
        ...
        in
        env_lookup_by_name Env.s_any (UnitName.to_string name) env >>= function
