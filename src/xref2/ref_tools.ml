...
type label_parent_lookup_result =
  Resolved.LabelParent.t * Env.t * [ `S of Component.Signature.t | `CS of Component.ClassSignature.t ]
...
let signature_lookup_result_of_label_parent : label_parent_lookup_result -> signature_lookup_result option =
  fun (rr,env,c) ->
    match (rr, c) with
    | `Identifier (#Odoc_model.Paths.Identifier.Signature.t) as rr', `S s -> Some (rr', env, s)
    | `SubstAlias _ | `Module _ | `Canonical _ | `ModuleType _  as rr', `S s -> Some (rr', env, s)
    | _ -> None
...
let rec resolve_type_reference : Env.t -> Type.t -> type_lookup_result option =
  ...
  | `Dot (parent, name) -> begin
    ...
    >>= fun (parent', _, sg) -> 
    Component.Find.opt_type_in_sig sg name
    >>= fun t -> return (`Type (parent', name), t)
    end
  | `Class (parent, name) ->
    resolve_signature_reference env parent ~add_canonical:true >>= fun (parent', _, sg) ->
...
and resolve_resolved_type_reference : Env.t -> Resolved.Type.t -> type_lookup_result =
  ...
  | `Type (parent, name) ->
    resolve_resolved_signature_reference env parent ~add_canonical:true |> fun (parent', _, sg) ->
...
and resolve_resolved_signature_reference : Env.t -> Resolved.Signature.t ->  add_canonical:bool -> signature_lookup_result =
...
| `Identifier i ->
  let sg = 
    ...
    | `Parameter _
    | `Result _
    | `Root _ as i' -> Tools.signature_of_module_nopath env (Env.lookup_module i' env) |> prefix_signature (`Identifier i')
    | `ModuleType _ as i' -> Tools.signature_of_module_type_nopath env (Env.lookup_module_type i' env) |> prefix_signature (`Identifier i')
  in
  let env = Env.open_component_signature (Resolved.Signature.identifier r) sg env in
  (r, env, sg)
| `Module (_, _) 
| `Canonical _
| `SubstAlias (_, _) as r' ->
    let (_, m) = resolve_resolved_module_reference env r' ~add_canonical in r, env, Tools.signature_of_module_nopath env m |> prefix_signature r
| `ModuleType (_, _) as r' ->
    let (_, m) = resolve_resolved_module_type_reference env r' in r, env, Tools.signature_of_module_type_nopath env m |> prefix_signature r'

and resolve_resolved_module_reference : Env.t -> Resolved.Module.t -> add_canonical:bool -> module_lookup_result =
  fun env r ~add_canonical ->
    match r with
    | `Identifier i ->
      ...
      r', m
    | `Module (parent, name) ->
      let (_, _, sg) = resolve_resolved_signature_reference env parent ~add_canonical in
      let m = Component.Find.module_in_sig sg name in
      let r' = if add_canonical then add_canonical_path env m r else r in
      r', m 
    | `Canonical (p, _) ->
      let _, m = resolve_resolved_module_reference env p ~add_canonical in r, m
    | `SubstAlias (_, p) ->
      let _, m = resolve_resolved_module_reference env p ~add_canonical in r, m
...
and resolve_resolved_module_type_reference: Env.t -> Resolved.ModuleType.t -> module_type_lookup_result =
  fun env r ->
    ...
    | `ModuleType (parent, name) ->
      let (_, _, sg) = resolve_resolved_signature_reference env parent ~add_canonical:true in
      r, Component.Find.module_type_in_sig sg name

and resolve_module_reference : Env.t -> Module.t -> add_canonical:bool -> module_lookup_result option =
  let open Tools.OptionMonad in
  fun env r ~add_canonical ->
    match r with
    | `Resolved r ->
    ...
    | `Dot (parent, name) ->
      ...
      >>= signature_lookup_result_of_label_parent 
      >>= fun (parent', _, sg) -> Component.Find.opt_module_in_sig sg name
      >>= fun m ->
      let r' = if add_canonical then add_canonical_path env m (`Module (parent', name)) else (`Module (parent', name)) in
      return (r', m)
    | `Module (parent, name) ->
    ...
    | `Root (name, _) -> begin
        ...
        | Some (`Module (id, m)) ->
          let r' = if add_canonical then add_canonical_path env m (`Identifier id) else (`Identifier id) in
          return (r', m)
        | None -> begin
          let x = Env.lookup_root_module name env in
          match x with | Some (Env.Resolved (id,m)) ->
            let r' = if add_canonical then add_canonical_path env m (`Identifier id) else (`Identifier id) in
            return (r', m) | _ -> None
        end
      end

      and resolve_module_type_reference : Env.t -> ModuleType.t -> add_canonical:bool -> module_type_lookup_result option =
  let open Tools.OptionMonad in
  fun env r ~add_canonical ->
    match r with
    | `Resolved r ->
    ...
    | `Dot (parent, name) ->
      resolve_label_parent_reference env parent ~add_canonical
      >>= signature_lookup_result_of_label_parent 
      >>= fun (parent', _, sg) -> Component.Find.opt_module_type_in_sig sg name
...
and resolve_label_parent_reference : Env.t -> LabelParent.t -> add_canonical:bool -> label_parent_lookup_result option =
      let open Tools.OptionMonad in
      fun env r ~add_canonical ->
      let label_parent_res_of_sig_res : signature_lookup_result option -> label_parent_lookup_result option = fun x ->
        x
        >>= (fun (r',env,sg) ->
      ...
      in
        ...
        | `Root (_, #Odoc_model.Paths_types.Reference.tag_module) as sr ->
            resolve_signature_reference env sr ~add_canonical |> label_parent_res_of_sig_res
        | `Dot (parent, name) ->
          resolve_label_parent_reference env parent ~add_canonical
          >>= signature_lookup_result_of_label_parent
          >>= fun (parent', env, sg) ->
          choose [
            (fun () -> Component.Find.opt_module_in_sig sg name >>= fun m ->
              let r' = if add_canonical then add_canonical_path env m (`Module (parent', name)) else (`Module (parent', name)) in
              let sg = Tools.signature_of_module_nopath env m |> prefix_signature (r' :> Resolved.Signature.t) in
              let env = Env.open_component_signature (Resolved.Signature.identifier (r' :> Resolved.Signature.t)) sg env in
              return ((r' :> Resolved.LabelParent.t), env, `S sg));
            (fun () -> Component.Find.opt_module_type_in_sig sg name >>= fun m ->
              let r' = `ModuleType (parent', name) in
              let sg = Tools.signature_of_module_type_nopath env m |> prefix_signature r' in
              let env = Env.open_component_signature (Resolved.Signature.identifier r') sg env in
              return (r', env, `S sg));]
        | _ -> None
...
and resolve_signature_reference : Env.t -> Signature.t -> add_canonical:bool -> signature_lookup_result option =
  let open Tools.OptionMonad in
  fun env r ~add_canonical ->
    match r with
    | `Resolved r -> Some (resolve_resolved_signature_reference env r ~add_canonical)
    | `Root (name, _) -> 
      choose [
        (fun () ->
          Env.lookup_module_by_name name env >>= fun (`Module (id, m)) ->
          let sg = Tools.signature_of_module_nopath env m |> prefix_signature (`Identifier (id :> Identifier.Signature.t)) in
          let env = Env.open_component_signature (id :> Identifier.Signature.t) sg env in
        ...
        (fun () ->
          ...
          | (Env.Resolved (id,m)) ->
            ...
            let sg = Tools.signature_of_module_nopath env m |> prefix_signature identifier in
            let env = Env.open_component_signature (id :> Identifier.Signature.t) sg env in
        ...
        (fun () ->
          ...
          let identifier = `Identifier (id :> Identifier.Signature.t) in
          let sg = Tools.signature_of_module_type_nopath env m |> prefix_signature identifier in
          let env = Env.open_component_signature (id :> Identifier.Signature.t) sg env in
          return (`Identifier (id :> Identifier.Signature.t), env, sg))
      ]
    | `Dot (parent, name) ->
      resolve_label_parent_reference env parent ~add_canonical
          >>= signature_lookup_result_of_label_parent
          >>= fun (parent', env, sg) -> Component.Find.opt_module_in_sig sg name
          >>= fun m ->
             let r' = `Module (parent', name) in
             let sg = Tools.signature_of_module_nopath env m |> prefix_signature r' in
             let env = Env.open_component_signature (Reference.Resolved.Signature.identifier r') sg env in
             return (r', env, sg)
    |`Module (parent,name) ->
      resolve_signature_reference env parent ~add_canonical
        >>= fun (parent', env, sg) -> Component.Find.opt_module_in_sig sg name
  ...
  | `ModuleType (parent,name) ->
      resolve_signature_reference env parent ~add_canonical
        >>= fun (parent', env, sg) -> Component.Find.opt_module_type_in_sig sg name
        >>= fun m ->
        let r' = `ModuleType (parent', name) in
        let sg = Tools.signature_of_module_type_nopath env m |> prefix_signature r' in
        let env = Env.open_component_signature (Reference.Resolved.Signature.identifier r') sg env in
...
and resolve_value_reference : Env.t -> Value.t -> value_lookup_result option =
  ...
  fun env r ->
    ...
    | `Dot (parent, name) -> begin
      resolve_label_parent_reference env parent ~add_canonical:true
          >>= signature_lookup_result_of_label_parent
          >>= fun (parent', _, sg) -> Component.Find.opt_value_in_sig sg name
...
and resolve_label_reference : Env.t -> Label.t -> Resolved.Label.t option =
    ...
    fun env r ->
      ...
      | `Dot (parent, name) -> begin
        resolve_label_parent_reference env parent ~add_canonical:true >>= fun (p,_,sg) ->
        match sg with
...
and resolve_reference : Env.t -> t -> Resolved.t option =
    ...
    | `Root (_, `TModule)
    | `Module (_,_) as r ->
        resolve_module_reference env r ~add_canonical:true >>= fun (x,_) -> return (x :> Resolved.t)
    | `Root (_,`TModuleType)
    | `ModuleType (_,_) as r ->
        resolve_module_type_reference env r ~add_canonical:true >>= fun (x, _) -> return (x :> Resolved.t)
    | `Root (_, `TType) 
    | `Type (_,_) as r ->
    ...
    | `Dot (_,_) as r ->
        choose 
          ...
          ; (fun () -> resolve_module_reference env r ~add_canonical:true >>= fun (x,_) -> return (x :> Resolved.t))
          ; (fun () -> resolve_module_type_reference env r ~add_canonical:true >>= fun (x,_) -> return (x :> Resolved.t))
...
and add_canonical_path env m p : Resolved.Module.t =
  ...
  | p ->
    ...
    | Some (_, cr) -> (
        Format.fprintf Format.err_formatter "Handling canonical path for %a (cr=%a)\n%!" (Component.Fmt.model_resolved_reference) (p :> Resolved.t) Component.Fmt.model_reference (cr :> Reference.t);
        ...
        | exception _e ->
            ...
            Component.Fmt.model_resolved_reference (p :> Resolved.t) (Printexc.get_backtrace ());
            p )
    | None ->
...
let _ = Tools.resolve_module_ref := resolve_module_reference ~add_canonical:false