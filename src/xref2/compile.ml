...
(* First round of resolving only attempts to resolve paths and fragments, and then only those
   ...
   and class_type env c =
     ...
     in
     let expansion =
       Some (Lang_of.class_signature Lang_of.empty
         ...
         | Some expr ->
           ...
           let env, expansion =
              try 
                ...
                env, Some e
              with Tools.OpaqueModule -> env, None in
           expansion,
             Some
   ...
   and expansion : Env.t -> Module.expansion -> Module.expansion =
    ...
    and functor_argument_opt : Env.t -> FunctorArgument.t option -> FunctorArgument.t option =
        fun env arg_opt ->
          Component.Opt.map (functor_argument env) arg_opt
