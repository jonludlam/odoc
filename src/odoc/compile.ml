...
let lookup_implementation_of_cmti source_id_opt id intf_file =
  let input_file = Fs.File.set_ext ".cmt" intf_file in
  if Fs.File.exists input_file then
    let filename = Fs.File.to_string input_file in
    Odoc_loader.read_cmt_infos source_id_opt id ~filename |> Error.raise_errors_and_warnings
  else (
    ...
    | `Cmti ->
        ...
        let cmt_infos =
          if should_read_impl_shape then
            lookup_implementation_of_cmti source unit.id input_file
          else None
        in
        (unit, cmt_infos)
    | `Cmt ->
        Odoc_loader.read_cmt ~make_root ~parent ~filename ~source_id_opt:source
  ...
  let source_info =
    ...
    | Some id ->
        let infos =
          ...
          | Some (_, source_info) -> source_info
          | _ -> []
        in
