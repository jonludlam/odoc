...
module Indexing = struct
  ...
  let output_file ~dst marshall =
    ...
    | Some file, `JSON when not ((Fpath.has_ext "json" (Fpath.v file)) || ((Fpath.has_ext "js" (Fpath.v file)))) ->
      Error
          (`Msg
  ...
  let cmd =
    ...
    in
    let simplified_json =
      let doc = "whether to simplify the json file. Only has an effect in json output mode." in
      Arg.(value & flag & info ~doc [ "simplified-json" ])
    in
    let wrap_json =
      let doc = "whether to wrap the json file. Only has an effect in json output mode." in
