...
module Compile : sig
  ...
  let compile hidden directories resolve_fwd_refs dst package_opt parent_opt open_modules children input warn_error =
    let env =
      Env.create ~important_digests:(not resolve_fwd_refs) ~directories ~open_modules
    in
    let parent_file_opt =
      match parent_opt with
      | Some f -> Some (Fs.File.of_string f)
      | None -> None
    in
    let input = Fs.File.of_string input in
    let output = output_file ~dst ~input in
    Fs.Directory.mkdir_p (Fs.File.dirname output);
    Compile.compile ~env ~parent_file_opt ~package_opt ~hidden ~children ~output ~warn_error input
...
module Depends = struct
  ...
  module Link = struct
    let rec fmt_page pp page =
      match page with
      | `Page name -> Format.fprintf pp "%a" Odoc_model.Names.PageName.fmt name
      | `SubPage (parent, name) -> Format.fprintf pp "%a/%a" fmt_page parent Odoc_model.Names.PageName.fmt name
