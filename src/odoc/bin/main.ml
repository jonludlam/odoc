...
let warnings_options =
  ...
  let suppress_warnings =
    let doc = 
      ...
      that would be generated resolving the references defined in this unit should \
      be ignored if they end up in expansions in other units."
    in
    let env = Arg.env_var "ODOC_SUPPRESS_WARNINGS" ~doc in
    Arg.(value & flag & info ~docs ~doc ~env [ "suppress-warnings" ])
  in
  Term.(
    const (fun warn_error print_warnings enable_missing_root_warning suppress_warnings ->
    ...
    $ warn_error $ print_warnings $ enable_missing_root_warning $ suppress_warnings)
...
module Make_renderer (R : S) : sig
  ...
  module Targets = struct
    let list_targets output_dir directories extra odoc_file =
      ...
      in
      let warnings_options =
        { Odoc_model.Error.warn_error = false; print_warnings = false; suppress_warnings = false }
      in
