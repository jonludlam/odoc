...
module Compile : sig
  ...
  let compile hidden directories resolve_fwd_refs dst package_opt parent_opt open_modules input warn_error =
    let env =
    ...
    Compile.compile ~env ~parent_file_opt ~package_opt ~hidden ~output ~warn_error input
