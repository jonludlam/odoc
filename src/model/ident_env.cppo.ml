...
let add_module parent id env =
  ...
  let ident : Id.Module.t = `Module(parent, ModuleName.of_string name) in
  let module_ = if should_be_hidden name then `Hidden (`Identifier ident) else `Identifier ident in
  let all = Ident.find_all name env.module_ids in
  let shadowing =
