...
let packages ~dirs ~extra_paths ~remap ~gen_indices (pkgs : Packages.t list) : t list =
  ...
  let index_of pkg =
    ...
    let simplified_path = Some Fpath.(pkg_dir / "index.js") in
    let doc_dir = doc_dir pkg in
    let sidebar =
      let output_file = Fpath.(index_dir / pkg.name / Odoc.sidebar_filename) in
      { output_file; json = false; doc_dir }
    in 
    {
      roots;
      output_file;
