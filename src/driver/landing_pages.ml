...
let make_rel dir l =
  List.map (fun (x, y) -> (x, Fpath.(dir // y))) l
...
module PackageLanding = struct
  ...
  let page ~odoc_dir ~odocl_dir ~mld_dir ~output_dir ~pkg =
    ...
    let pkg_args =
      { pages = make_rel output_dir pages_rel; libs = []; pages_linked = make_rel odocl_dir pages_rel; libs_linked = [] }
    in
...
module PackageList = struct
  ...
  let page ~mld_dir ~odoc_dir ~odocl_dir ~output_dir all =
    ...
    let pkg_args =
      { pages = make_rel odoc_dir pages_rel; libs = []; pages_linked = make_rel odocl_dir pages_rel; libs_linked = [] }
    in
...
let of_package ~mld_dir ~odoc_dir ~odocl_dir ~output_dir pkg =
  let library_pages =
    List.map
      (LibraryLanding.page ~pkg ~odoc_dir ~odocl_dir ~mld_dir ~pkg_dir:pkg.pkg_dir ~output_dir)
      pkg.libraries
  in
