...
let rec shape_of_module_path lookup_shape :
    Odoc_model.Paths.Path.t -> Shape.t option =
  let proj parent kind name =
    let item = Shape.Item.make name kind in
    match shape_of_module_path lookup_shape (parent :> Odoc_model.Paths.Path.t) with
    | Some shape -> Some (Shape.proj shape item)
    | None -> None
  in
  fun (path : Odoc_model.Paths.Path.t) ->
    ...
    | `Resolved _ -> None
    | `Root name ->
        lookup_shape name >>= fun (_, (shape, _)) ->
        Some shape
    | `Forward _ -> None
    | `Dot (parent, name) -> proj (parent :> Odoc_model.Paths.Path.t) Kind.Module name
    | `Apply (parent, arg) ->
        shape_of_module_path lookup_shape (parent :> Odoc_model.Paths.Path.t) >>= fun parent ->
        shape_of_module_path lookup_shape (arg :> Odoc_model.Paths.Path.t) >>= fun arg ->
        Some (Shape.app parent ~arg)
    | `Identifier (id, _) -> shape_of_id lookup_shape (id :> Odoc_model.Paths.Identifier.NonSrc.t)

let shape_of_value_path lookup_shape :
...
let lookup_shape : (string -> (Lang.Compilation_unit.t * t) option) ->
    Shape.t -> Identifier.SourceLocation.t option = fun lookup_unit query ->
      (* Format.eprintf "looking up shape: %a\n%!" Shape.print query; *)
      let module Reduce = Shape.Make_reduce (struct
        type env = unit
        let fuel = 10
        let read_unit_shape ~unit_name =
          (* Format.eprintf "read_unit_shape: %s\n%!" unit_name; *)
          match lookup_unit unit_name with
          | Some (_, (shape, _)) -> (*Format.eprintf "Some!\n%!";*) Some shape
      ...
      result >>= fun result ->
      result.uid >>= fun uid ->
      Uid.unpack_uid uid >>= fun (unit_name, _id) ->
      lookup_unit unit_name >>= fun (unit, (_, uid_to_anchor)) ->
      (* Format.eprintf "looking up %a in %s\n%!" Shape.Uid.print uid unit_name; *)
      match Shape.Uid.Map.find_opt uid uid_to_anchor with
      | Some x -> (*Format.eprintf "Found it!\n%!";*) Some x
      | None -> match unit.source_info with | Some si -> Some (MkId.source_location_mod si.id) | None -> None
...
let lookup_value_path : (string -> (Lang.Compilation_unit.t * t) option) ->
  Path.Value.t ->
  Identifier.SourceLocation.t option = fun lookup_unit path ->
  match shape_of_value_path lookup_unit path with
  | None -> None
  | Some query -> lookup_shape lookup_unit query
...
module Analysis2 = struct
  ...
  let env_wrap : (Ident_env.t -> Ident_env.t) -> env -> env = fun f (env, uid_to_loc) ->
    (f env, uid_to_loc)

  let get_env : env -> Ident_env.t = fun (env, _) -> env

  let get_uid_to_loc : env -> Warnings.loc Shape.Uid.Tbl.t = fun (_, uid_to_loc) -> uid_to_loc
  ...
  and pattern : type a. env -> a general_pattern -> _ = fun env p ->
    ...
    | Tpat_any -> []
    | Tpat_var (id, loc) -> (match maybe_localvalue id loc.loc with | Some x -> [x] | None -> [])
    | Tpat_alias (p, id, loc) -> (match maybe_localvalue id loc.loc with | Some x -> x :: pattern env p | None -> pattern env p)
    | Tpat_constant _ -> []
    | Tpat_tuple ps ->
        List.fold_left
          (fun acc p -> List.rev_append (pattern env p) acc)
          [] ps
  ...
  | { exp_desc = Texp_ident (p, _, value_description); exp_loc; _ } -> (
      ...
      match Shape.Uid.Tbl.find_opt (get_uid_to_loc env) value_description.val_uid with
      | Some _ ->
        [DefJmp value_description.val_uid, pos_of_loc exp_loc]
      | None when not exp_loc.loc_ghost -> (
        try
          let path = Ident_env.Path.read_value (get_env env) p in
          [UnresolvedJmp (path :> Odoc_model.Paths.Path.Value.t), pos_of_loc exp_loc]
        with _ -> (
          match p with
          | Pident id ->
            (* Format.eprintf "Found localvalue %s\n%!" (
              Ident.name id
            ); *)
            [LocalValue id, pos_of_loc exp_loc]
          | _ ->
            []))
      | None -> [])
  | { exp_desc = Texp_constant _; _ } -> []
  | { exp_desc = Texp_let (_, vbs, e); _ } -> List.concat_map (value_binding env) vbs @ expression env e
  | { exp_desc = Texp_function f; _ } -> List.concat_map (case env) f.cases
  | { exp_desc = Texp_match (e, cases, _); _ } -> expression env e @ List.concat_map (case env) cases
  | { exp_desc = Texp_try (e, cases); _ } -> expression env e @ List.concat_map (case env) cases
  | { exp_desc = Texp_tuple es; _ } -> List.concat_map (expression env) es
  | { exp_desc = Texp_construct (_, _, es); _ } -> List.concat_map (expression env) es
  ...
  | { exp_desc = Texp_record {fields; extended_expression; _ }; _ } ->
    let e = match extended_expression with None -> [] | Some expr -> expression env expr in
    e @ List.concat_map (record_fields env) (Array.to_list fields)
  | { exp_desc = Texp_field (e, _, _); _ } -> expression env e
  | { exp_desc = Texp_setfield (e1, _, _, e2); _ } -> expression env e1 @ expression env e2
  ...
  | { exp_desc = Texp_sequence (e1, e2); _ } -> expression env e1 @ expression env e2
  | { exp_desc = Texp_while (e1, e2); _ } -> expression env e1 @ expression env e2
  | { exp_desc = Texp_for (id, p, e1, e2, _, e3); _ } ->
     (LocalValue id, pos_of_loc p.ppat_loc) :: expression env e1 @ expression env e2 @ expression env e3
  ...
  | { exp_desc = Texp_override (_, es); _ } -> List.concat_map (fun (_, _, e) -> expression env e) es
  ...
  | { exp_desc = Texp_apply (e, es); _ } -> expression env e @ List.concat_map (function (_, Some e) -> expression env e | _ -> []) es


  and binding_op env = function
    | { bop_exp; _ } ->
  ...
  and case : type a. env -> a Typedtree.case -> _ = fun env c ->
    pattern env c.c_lhs @
  ...
  and module_bindings env parent mbs =
    let items =
      List.fold_left
        (fun acc vb ->
          List.rev_append (module_binding env parent vb) acc)
        [] mbs
    in
    List.rev items
    ...
    | Tmod_functor(parameter, res) ->
        let _f_parameter, env =
          ...
          | Named (id_opt, _, _arg) ->
              let name, env =
                match id_opt with
                | Some id -> Ident.name id, env_wrap (Ident_env.add_parameter parent id (ModuleName.of_ident id)) env
                | None -> "_", env
              in
  ...
  let local_def_anchors =
    List.filter_map (function
      | LocalDefinition id, (start,_) ->
        let name = Odoc_model.Names.LocalName.make_std (Printf.sprintf "local_%s_%d" (Ident.name id) start) in
        let identifier = Odoc_model.Paths.Identifier.Mk.source_location_int (source_id, name) in
        Some (id, identifier)
      | _ -> None) poses in
  let poses =
    List.filter_map (function
      | LocalDefinition id, loc -> Some (Odoc_model.Lang.Source_info.Definition (List.assoc id local_def_anchors), loc)
      | LocalValue uniq, loc -> (
      ...
      | DefJmp x, loc -> (
        ...
        | None -> None)
      | UnresolvedJmp p, loc -> Some (Unresolved p, loc)) poses
  in
  let defs = Shape.Uid.Map.fold (fun uid id acc ->
    let loc_opt = Shape.Uid.Tbl.find_opt uid_to_loc uid in
    match loc_opt with
    | Some loc -> (Odoc_model.Lang.Source_info.Definition id, pos_of_loc loc) :: acc
    | _ -> acc) uid_to_id [] in
  defs @ poses
...
let anchor_of_identifier id =
  ...
  List.filter_map (fun (x, y) ->
    ...
    | `Src -> None
    | `Asset -> None
    | _ -> Some (Printf.sprintf "%s-%s" (string_of_full_name_ty x) y)) full_name |> 
  List.tl |> String.concat "."

let of_cmt (source_id_opt : Odoc_model.Paths.Identifier.SourcePage.t option) (id : Odoc_model.Paths.Identifier.RootModule.t) (cmt : Cmt_format.cmt_infos) =
  ...
  | Some source_id, Cmt_format.Implementation structure, Some shape ->
      ...
      let vs = Analysis2.structure (env, uid_to_loc) (id :> Odoc_model.Paths.Identifier.Signature.t) structure in
      let uid_to_loc_map = Shape.Uid.Tbl.to_map uid_to_loc in
      let uid_to_id : Odoc_model.Paths.Identifier.SourceLocation.t Shape.Uid.Map.t = Shape.Uid.Map.filter_map (fun uid loc ->
        let identifier = Ident_env.identifier_of_loc env loc in
        let anchor =
          match identifier with 
          | Some x -> Some (Odoc_model.Names.DefName.make_std (anchor_of_identifier x))
          | None ->
            match uid with
            | Compilation_unit _ -> None
            | Item { id; _ } -> 
              let name = Odoc_model.Names.DefName.make_std (Printf.sprintf "def_%d" id) in
              Some name
            | _ -> None in
        match anchor with
        | Some a -> Some (Odoc_model.Paths.Identifier.Mk.source_location (source_id, a) :> Odoc_model.Paths.Identifier.SourceLocation.t)
        | None -> None
          ) uid_to_loc_map in

      Some (shape, uid_to_id), postprocess_poses source_id vs uid_to_id uid_to_loc
  | None, _, Some shape ->
