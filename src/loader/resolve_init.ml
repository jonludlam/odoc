...
module Resolve = struct
  ...
  let lookup_signature env name : Paths.Reference.Signature.t option =
    let all = 
      (Ident_env.lookup_module_by_name env name :> Paths.Identifier.Signature.t list) @
      (Ident_env.lookup_module_type_by_name env name :> Paths.Identifier.Signature.t list)
    in
    match all with
    | x :: _ -> Some (`Resolved (`Identifier x))
    | _ -> None
 
  let lookup_any env name : Paths.Reference.t option =
    let all = 
      (Ident_env.lookup_module_by_name env name :> Paths.Identifier.t list) @
      (Ident_env.lookup_module_type_by_name env name :> Paths.Identifier.t list) @
  ...
  let rec signature env (x : Odoc_model.Paths.Reference.Signature.t) : Odoc_model.Paths.Reference.Signature.t =
    match x with
    | `Resolved _ -> x
    | `Root (r, t) -> (
        match t with
  ...
  and label_parent env (x : Odoc_model.Paths.Reference.LabelParent.t) : Odoc_model.Paths.Reference.LabelParent.t =
    match x with 
    | `Resolved _ -> x
    | `Dot (p, n) -> `Dot (label_parent env p, n)
    | `Root (r,t) -> (
  ...
  let do_resolve env (x : Odoc_model.Paths.Reference.t) : Odoc_model.Paths.Reference.t =
    match x with
    | `Resolved _ -> x
    | `Root (_r, (`TModule
      | `TModuleType
...
let with_location : ('a -> 'b) -> 'a Comment.with_location -> 'b Comment.with_location =
...
let rec inline_element env (x : Comment.inline_element) : Comment.inline_element =
  ...
  | `Styled (s, i) -> `Styled (s, List.map (with_location (inline_element env)) i)
  | `Link _ -> x

let paragraph env = List.map (with_location (inline_element env))

let rec nestable_block_element : Ident_env.t -> Comment.nestable_block_element -> Comment.nestable_block_element =
  fun env -> function
  ...
  | `Modules _ as x -> x
  | `Table _ as x -> x
  | `List (kind, items) -> `List (kind, List.map (List.map (with_location (nestable_block_element env))) items)
  | `Media _ as x -> x

let tag : Comment.tag -> Comment.tag =
  function
  | x -> x

let block_element env : Comment.block_element -> Comment.block_element =
    function
    | #Comment.nestable_block_element as e -> (nestable_block_element env e :> Comment.block_element)
