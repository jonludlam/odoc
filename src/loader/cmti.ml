...
let rec read_core_type env container ctyp =
  let open TypeExpr in
    ...
    | Ttyp_constr(p, _, params) ->
        let p = Env.Path.read_type env.e p in
    ...
    | Ttyp_class(p, _, params) ->
        let p = Env.Path.read_class_type env.e p in
        let params = List.map (read_core_type env container) params in
          Class(p, params)
    | Ttyp_alias(typ, var) ->
...
#endif
                let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container attributes in
    ...
    | Ttyp_package {pack_path; pack_fields; _} ->
        ...
        let path = Env.Path.read_module_type env.e pack_path in
        let substitutions =
          List.map
            (fun (frag, typ) ->
...
let read_value_description env parent vd =
  ...
  let id = Env.find_value_identifier env.e vd.val_id in
  let source_loc = None in
  let container =
    (parent : Identifier.Signature.t :> Identifier.LabelParent.t)
  in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container vd.val_attributes in
...
let read_label_declaration env parent label_parent ld =
  ...
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw label_parent ld.ld_attributes in
...
let read_constructor_declaration env parent cd =
  let open TypeDecl.Constructor in
  let id = Ident_env.find_constructor_identifier env.e cd.cd_id in
  let container = (parent :> Identifier.FieldParent.t) in
  let label_container = (container :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw label_container cd.cd_attributes in
  let args =
...
let read_type_declaration env parent decl =
  let open TypeDecl in
  let id = Env.find_type_identifier env.e decl.typ_id in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc, canonical = Doc_attr.attached ~env:env.e ~suppress_warnings:env.sw Odoc_model.Semantics.Expect_canonical container decl.typ_attributes in
    ...
    List.fold_left
      (fun (acc, recursive) decl ->
        ...
        then (acc, recursive)
          ...
          let comments =
            Doc_attr.standalone_multiple container ~suppress_warnings:env.sw decl.typ_attributes in
...
let read_extension_constructor env parent ext =
  ...
  let id = Env.find_extension_identifier env.e ext.ext_id in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.FieldParent.t) in
  let label_container = (container :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw label_container ext.ext_attributes in
...
let read_type_extension env parent tyext =
  ...
  let type_path = Env.Path.read_type env.e tyext.tyext_path in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container tyext.tyext_attributes in
...
let read_exception env parent (ext : extension_constructor) =
  ...
  let id = Env.find_exception_identifier env.e ext.ext_id in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.FieldParent.t) in
  let label_container = (container :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw label_container ext.ext_attributes in
...
let rec read_class_type_field env parent ctf =
  ...
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container ctf.ctf_attributes in
  ...
  | Tctf_attribute attr ->
      match Doc_attr.standalone container ~suppress_warnings:env.sw attr with
      | None -> None
...
let read_class_type_declaration env parent cltd =
  let open ClassType in
  let id = Env.find_class_type_identifier env.e cltd.ci_id_class_type in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag container ~env:env.e ~suppress_warnings:env.sw cltd.ci_attributes in
...
let read_class_type_declarations env parent cltds =
  ...
  List.fold_left begin fun (acc,recursive) cltd ->
    let comments = Doc_attr.standalone_multiple container ~suppress_warnings:env.sw cltd.ci_attributes in
    let comments = List.map (fun com -> Comment com) comments in
...
let read_class_description env parent cld =
  let open Class in
  let id = Env.find_class_identifier env.e cld.ci_id_class in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag container ~env:env.e ~suppress_warnings:env.sw cld.ci_attributes in
...
let read_class_descriptions env parent clds =
  ...
  List.fold_left begin fun (acc, recursive) cld ->
    let comments = Doc_attr.standalone_multiple container ~suppress_warnings:env.sw cld.ci_attributes in
    let comments = List.map (fun com -> Comment com) comments in
...
let rec read_with_constraint env global_parent parent (_, frag, constr) =
  ...
  let open ModuleType in
    ...
    | Twith_modsubst(p, _) ->
        let frag = Env.Fragment.read_module frag.Location.txt in
        let p = Env.Path.read_module env.e p in
          ModuleSubst(frag, p)
...
and read_module_type env parent label_parent mty =
  let open ModuleType in
    ...
    | Tmty_ident(p, _) -> Path { p_path = Env.Path.read_module_type env.e p; p_expansion = None }
    | Tmty_signature sg ->
        ...
        let f_parameter, env =
          ...
          | Named (id_opt, _, arg) ->
            let id, env =
              ...
              | Some id ->
                 let e' = Env.add_parameter parent id (ModuleName.of_ident id) env.e in
                 let env = {env with e = e'} in
...
and read_module_type_declaration env parent mtd =
  let open ModuleType in
  let id = Env.find_module_type env.e mtd.mtd_id in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc, canonical = Doc_attr.attached ~env:env.e ~suppress_warnings:env.sw Odoc_model.Semantics.Expect_canonical container mtd.mtd_attributes in
  let expr, canonical =
...
and read_module_declaration env parent md =
  ...
  | None -> None
  | Some id ->
  let mid = Env.find_module_identifier env.e id in
#else
  let mid = Env.find_module_identifier env.e md.md_id in
#endif
  let id = (mid :> Identifier.Module.t) in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc, canonical = Doc_attr.attached ~env:env.e ~suppress_warnings:env.sw Odoc_model.Semantics.Expect_canonical container md.md_attributes in
  let type_, canonical =
    match md.md_type.mty_desc with
    | Tmty_alias (p, _) -> (Alias (Env.Path.read_module env.e p, None), canonical)
    | _ ->
        let expr, canonical =
          read_module_type_maybe_canonical env
            (id :> Identifier.Signature.t)
...
and read_module_declarations env parent mds =
  ...
  List.fold_left
    (fun (acc, recursive) md ->
      let comments = Doc_attr.standalone_multiple container ~suppress_warnings:env.sw md.md_attributes in
      let comments = List.map (fun com -> Comment com) comments in
...
and read_module_equation env p =
  let open Module in
    Alias (Env.Path.read_module env.e p, None)
...
and read_module_substitution env parent ms =
  ...
  let id = Env.find_module_identifier env.e ms.ms_id in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc, () = Doc_attr.attached ~env:env.e ~suppress_warnings:env.sw Odoc_model.Semantics.Expect_none container ms.ms_attributes in
  let manifest = Env.Path.read_module env.e ms.ms_manifest in
  { id; doc; manifest }
...
and read_module_type_substitution env parent mtd =
  ...
  let id = Env.find_module_type env.e mtd.mtd_id in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc, () = Doc_attr.attached ~env:env.e ~suppress_warnings:env.sw Odoc_model.Semantics.Expect_none container mtd.mtd_attributes in
...
and read_include env parent incl =
  ...
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc, status = Doc_attr.attached ~env:env.e ~suppress_warnings:env.sw Odoc_model.Semantics.Expect_status container incl.incl_attributes in
...
and read_open env parent o =
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag container ~env:env.e ~suppress_warnings:env.sw o.open_attributes in
  #if OCAML_VERSION >= (4,8,0)
...
and read_signature :
 ...
 fun internal_tags env parent sg ->
  let e' = Env.add_signature_tree_items parent sg env.e in
  let env = { env with e = e' } in
  let items, (doc, doc_post), tags =
    let classify item =
      ...
      | _ -> None
    in
    Doc_attr.extract_top_comment ~env:env.e internal_tags ~classify parent sg.sig_items
  in
  let items =
    List.fold_left
      (fun items item ->
