    ...
    let typ =
      ...
      | Tpackage(p, frags, tyl) ->
        let eqs = List.combine frags tyl in
          ...
          let path = Env.Path.read_module_type env.e p in
          let substitutions =
            List.map
              (fun (frag,typ) ->
...
let read_value_description env parent id vd =
  ...
  let id = Env.find_value_identifier env.e id in
...
let read_label_declaration env parent ld =
  ...
  let doc =
    Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw
...
let read_constructor_declaration env parent cd =
  ...
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container cd.cd_attributes in
  let args =
...
let read_type_declaration env parent id decl =
  let open TypeDecl in
  let id = Env.find_type_identifier env.e id in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc, canonical =
    Doc_attr.attached ~env:env.e ~suppress_warnings:env.sw Odoc_model.Semantics.Expect_canonical container decl.type_attributes
  in
...
let read_extension_constructor env parent id ext =
  ...
  let id = Env.find_extension_identifier env.e id in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container ext.ext_attributes in
  let args =
    read_constructor_declaration_arguments env
...
let read_type_extension env parent id ext rest =
  ...
  let type_path = Env.Path.read_type env.e ext.ext_type_path in
...
let read_exception env parent id ext =
  ...
  let id = Env.find_exception_identifier env.e id in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container ext.ext_attributes in
    mark_exception ext;
...
let read_class_type_declaration env parent id cltd =
  ...
  let id = Env.find_class_type_identifier env.e id in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container cltd.clty_attributes in
...
let read_class_declaration env parent id cld =
  let open Class in
  let id = Env.find_class_identifier env.e id in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container cld.cty_attributes in
...
let rec read_module_type env parent (mty : Odoc_model.Compat.module_type) =
  let open ModuleType in
    match mty with
    | Mty_ident p -> Path {p_path = Env.Path.read_module_type env.e p; p_expansion=None }
        ...
        let f_parameter, env =
          ...
          | Named (id_opt, arg) ->
              let id, env = match id_opt with
                ...
                | Some id -> let e' = Env.add_parameter parent id (ModuleName.of_ident id) env.e in
                  Ident_env.find_parameter_identifier e' id, {env with e = e'} 
              in
    ...
    | Mty_alias p ->
        let t_original_path = Env.Path.read_module env.e p in
...
and read_module_type_declaration env parent id (mtd : Odoc_model.Compat.modtype_declaration) =
  let open ModuleType in
  let id = Env.find_module_type env.e id in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
and read_module_declaration env parent ident (md : Odoc_model.Compat.module_declaration) =
  let open Module in
  let id = (Env.find_module_identifier env.e ident :> Identifier.Module.t) in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  ...
  let type_ =
    ...
    | Mty_alias p -> Alias (Env.Path.read_module env.e p, None)
...
and read_signature_noenv env parent (items : Odoc_model.Compat.signature) =
  let rec loop (acc,shadowed) items =
    ...
    | Sig_value(id, v, _) :: rest ->
        ...
        let shadowed =
          if Env.is_shadowed env.e id
          then
            let identifier = Env.find_value_identifier env.e id in
    ...
    | Sig_type(id, decl, rec_status, _)::rest ->
        ...
        let shadowed =
          if Env.is_shadowed env.e id
          then
            let identifier = Env.find_type_identifier env.e id in
    ...
    | Sig_module (id, _, md, rec_status, _)::rest ->
          ...
          let shadowed =
            if Env.is_shadowed env.e id
            then
              let identifier = Env.find_module_identifier env.e id in
            let name =
              match identifier.iv with
              | `Module (_, n) -> n 
              | `Parameter (_, n) -> n
...
{ shadowed with s_modules = (Ident.name id, name) :: shadowed.s_modules }
    ...
    | Sig_modtype(id, mtd, _) :: rest ->
          ...
          let shadowed =
            if Env.is_shadowed env.e id
            then
              let identifier = Env.find_module_type env.e id in
            let name =
              match identifier.iv with
              | `ModuleType (_, n) -> n
            in
    ...
    | Sig_class(id, cl, rec_status, _) :: Sig_class_type _
      :: Sig_type _ :: rest ->
          ...
          let shadowed =
            if Env.is_shadowed env.e id
            then
              let identifier = Env.find_class_identifier env.e id in
            let name =
              match identifier.iv with
              | `Class (_, n) -> n
            in
    ...
    | Sig_class_type(id, cltyp, rec_status, _)::Sig_type _::rest ->
        ...
        let shadowed =
          if Env.is_shadowed env.e id
          then
            let identifier = Env.find_class_type_identifier env.e id in
          let name =
            match identifier.iv with
            | `ClassType (_, n) -> n
          in
...
and read_signature env parent (items : Odoc_model.Compat.signature) =
  let e' = Env.handle_signature_type_items parent items env.e in
