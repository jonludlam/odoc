...
let read_value_binding env parent vb =
  ...
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container vb.vb_attributes in
  ...
  let items =
    List.fold_left
      (fun acc vb ->
         let open Signature in
        let comments =
          Doc_attr.standalone_multiple container ~suppress_warnings:env.sw vb.vb_attributes in
...
let mk_class_comment = function
  | { Odoc_model.Comment.docs=[]; _} -> None
...
let rec read_class_field env parent cf =
  ...
  let open Odoc_model.Names in
  let container = (parent : Identifier.ClassSignature.t :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container (cf.cf_attributes) in
...
let read_class_declaration env parent cld =
  ...
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container cld.ci_attributes in
          ...
          | Named (id_opt, _, arg) ->
              let id, env =
                ...
                | Some id -> let e' = Env.add_parameter parent id (ModuleName.of_ident id) env.e in
                  Env.find_parameter_identifier e' id, {env with e=e'}
              in
...
#else
  let mid = Env.find_module_identifier env.e mb.mb_id in
#endif
  let id = (mid :> Identifier.Module.t) in
  let source_loc = None in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc, canonical = Doc_attr.attached ~env:env.e ~suppress_warnings:env.sw Odoc_model.Semantics.Expect_canonical container mb.mb_attributes in
  let type_, canonical =
    match unwrap_module_expr_desc mb.mb_expr.mod_desc with
    | Tmod_ident (p, _) -> (Alias (Env.Path.read_module env.e p, None), canonical)
    | _ ->
...
and read_module_bindings env parent mbs =
  ...
  List.fold_left
    (fun (acc, recursive) mb ->
      let comments = Doc_attr.standalone_multiple container ~suppress_warnings:env.sw mb.mb_attributes in
      let comments = List.map (fun com -> Comment com) comments in
...
and read_open env parent o =
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
  let doc = Doc_attr.attached_no_tag ~env:env.e ~suppress_warnings:env.sw container o.open_attributes in
  #if OCAML_VERSION >= (4,8,0)
...
and read_structure :
 ...
 fun internal_tags env parent str ->
  let e' = Env.add_structure_tree_items parent str env.e in
  ...
  let items, (doc, doc_post), tags =
    let classify item =
      ...
      | _ -> None
    in
    Doc_attr.extract_top_comment ~env:env.e internal_tags ~classify parent str.str_items
  in
  let items =
    List.fold_left
      (fun items item ->
