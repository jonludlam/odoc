    ...
    | Tmod_functor(id, _, arg, res) ->
        let f_parameter =
          ...
          | Some arg ->
              ...
              let id = `Parameter(parent, ModuleName.make_std name) in
          let arg = Cmti.read_module_type env id label_parent arg in
...
and read_include env parent incl =
  ...
  | _ -> 
    ...
    let fake_id = Identifier.Mk.module_ (parent, Odoc_model.Names.ModuleName.internal_of_string "OdocIncludedModule") in
