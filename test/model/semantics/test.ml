...
let parser_output_desc =
  ...
  Indirect
    ( Error.unpack_warnings,
      Record
        [
          F ("value", fst, Indirect (fst, Comment_desc.content));
...
let%expect_test _ =
  let module Raw_markup = struct
    ...
    let html_payload =
      ...
      [%expect
        {|
        {"value":[{"`Paragraph":[{"`Raw_markup":["html","<e>foo</e>"]}]}],"warnings":["File \"f.ml.mld\":\nPages (.mld files) should start with a heading."]} |}]
...
let%expect_test _ =
  let module Reference_component_kind = struct
    ...
    let canonical_something =
      ...
      [%expect
        {|
        {"value":[],"warnings":["File \"f.ml\", line 1, characters 0-14:\nUnexpected tag '@canonical' at this location."]} |}]

    let canonical_module =
      test "@canonical module-Foo";
      [%expect
        {|
        {"value":[],"warnings":["File \"f.ml\", line 1, characters 0-21:\nUnexpected tag '@canonical' at this location."]} |}]

    let canonical_path =
      test "@canonical Foo.Bar";
      [%expect
        {|
        {"value":[],"warnings":["File \"f.ml\", line 1, characters 0-18:\nUnexpected tag '@canonical' at this location."]} |}]
    ...
    let canonical_bad_parent =
      ...
      [%expect
        {|
        {"value":[],"warnings":["File \"f.ml\", line 1, characters 0-23:\nUnexpected tag '@canonical' at this location."]} |}]
