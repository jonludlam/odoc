...
module SomeMonad/29 :
  sig
    type t/34
    include r(Monad/30) with [r(root(Monad/30).t) = [a] resolved(t/34)]
      (sig :
...
module ComplexTypeExpr/28 :
  sig
    type t/37
    include r(Monad/30) with [r(root(Monad/30).t) = ([resolved(int) * a] resolved(t/37) * [a * resolved(int)] resolved(t/37))]
      (sig :
...
module Erase/27 :
  sig
    include r(Monad/30) with [r(root(Monad/30).t) = a]
...
module SwappedVars/53 :
  sig
    type t/59
    include r(Monad_2/54) with [r(root(Monad_2/54).t) = [b * a] resolved(t/59)]
      (sig :
...
module M/68 :
  sig
    type t/72
    include r(S/69) with [r(root(S/69).t) = [(alias (poly_var [ `A of (a * b) ]) b)] resolved(t/72)]
      (sig :
