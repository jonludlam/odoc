...
module SomeMonad/58 = : sig
  type t/60 ...
  include : r(Monad/59) with [r(root(Monad/59).t) = [a] r(t/60)] (sig =
    val map/61 ...
    val join/62 ...
     (removed=[]))
   (removed=[])end
module ComplexTypeExpr/57 = : sig
  type t/63 ...
  include : r(Monad/59) with [r(root(Monad/59).t) = ([r(int) * a] r(t/63) * [a * r(int)] r(t/63))] (sig =
    val map/64 ...
    val join/65 ...
     (removed=[]))
   (removed=[])end
module Erase/56 = : sig
  include : r(Monad/59) with [r(root(Monad/59).t) = a] (sig = val map/66 ...
                                                              val join/67 ...
                                                               (removed=[]))
   (removed=[])end
 (removed=[])
...
# compile {|
  ...
  |} ;;
- : Component.Signature.t =
module type Monad_2/109 ...
module SwappedVars/108 = : sig
  type t/110 ...
  include : r(Monad_2/109) with [r(root(Monad_2/109).t) = [b * a] r(t/110)] (sig =
    val map/111 ...
    val join/112 ...
    val both/113 ...
     (removed=[]))
...
module M/134 = : sig
  type t/136 ...
  include : r(S/135) with [r(root(S/135).t) = [(alias (poly_var [ `A of (a * b) ]) b)] r(t/136)] (sig =
    val map/137 ...
     (removed=[]))
   (removed=[])end
 (removed=[])
