...
| `OpaqueModule m ->
  ...
  and simple_expansion ppf (m : ModuleType.simple_expansion) =
    match m with
    | ModuleType.Signature sg -> Format.fprintf ppf ": sig@,@[<v 2>%a@]@,end" signature sg
    | Functor (arg, sg) ->
  ...
  and module_type_expr ppf mt =
    ...
    | Path { p_path; p_expansion=None; _ } ->
        path ppf (p_path :> path)
  ...
  and substitution ppf t =
    ...
    match t with
    | ModuleEq (frag, decl) ->
        Format.fprintf ppf "module %a = %a" fragment (frag :> frag) module_decl decl
    ...
    | ModuleTypeSubst (frag, mty) ->
        Format.fprintf ppf "module type %a := %a" fragment
          ...
          module_type_expr mty
    | TypeEq (frag, decl) ->
        Format.fprintf ppf "type %a%a" fragment (frag :> frag) type_equation decl
    | TypeSubst (frag, decl) ->
        Format.fprintf ppf "type %a%a" fragment (frag :> frag) type_equation decl
...
let print_short elt =
  ...
  match elt with
  | Element.Module m -> Format.fprintf Format.std_formatter "@[<v 2>module %a %a@]" identifier (m.id :> id) module_ m
  | Element.ModuleType m -> Format.fprintf Format.std_formatter "@[<v 2>module type %a %a@]" identifier (m.id :> id) module_type m
  | Element.Type t -> Format.fprintf Format.std_formatter "@[<v 2>type %a %a@]" identifier (t.id :> id) type_decl t
  | Element.Value v -> Format.fprintf Format.std_formatter "@[<v 2>val %a %a@]" identifier (v.id :> id) value v
